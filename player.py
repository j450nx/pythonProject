...
class HumanPlayer(Player):
    ...
    def get_move(self, game):
        ...
        while not valid_square:
            ...
            # it to an integer, and if it's not, then we say its invalid if that spot
            # is not available on the board, we also say its valid
            try:
                val = int(square)
...
class GeniusComputerPlayer(Player):
    ...
    def minimax(self, state, player):
        ...
        if state.current_winner == other_player:
            # we should return position AND score because need to keep track of the score
            # for minimax to work
        ...
        for possible_move in state.available_moves():
            ...
            # step 2: recurse using miniman to simulate a game after making that move
